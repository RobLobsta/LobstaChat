...
class LobstaChatLM {
    companion object {
        init {
            val logTag = LobstaChatLM::class.java.simpleName
            ...
            if (!isEmulated) {
                if (supportsArm64V8a()) {
                    if (isAtLeastArmV84 && hasSve && hasI8mm && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading liblobstachatlm_v8_4_fp16_dotprod_i8mm_sve.so")
                        System.loadLibrary("lobstachatlm_v8_4_fp16_dotprod_i8mm_sve")
                    } else if (isAtLeastArmV84 && hasSve && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading liblobstachatlm_v8_4_fp16_dotprod_sve.so")
                        System.loadLibrary("lobstachatlm_v8_4_fp16_dotprod_sve")
                    } else if (isAtLeastArmV84 && hasI8mm && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading liblobstachatlm_v8_4_fp16_dotprod_i8mm.so")
                        System.loadLibrary("lobstachatlm_v8_4_fp16_dotprod_i8mm")
                    } else if (isAtLeastArmV84 && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading liblobstachatlm_v8_4_fp16_dotprod.so")
                        System.loadLibrary("lobstachatlm_v8_4_fp16_dotprod")
                    } else if (isAtLeastArmV82 && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading liblobstachatlm_v8_2_fp16_dotprod.so")
                        System.loadLibrary("lobstachatlm_v8_2_fp16_dotprod")
                    } else if (isAtLeastArmV82 && hasFp16) {
                        Log.d(logTag, "Loading liblobstachatlm_v8_2_fp16.so")
                        System.loadLibrary("lobstachatlm_v8_2_fp16")
                    } else {
                        Log.d(logTag, "Loading liblobstachatlm_v8.so")
                        System.loadLibrary("lobstachatlm_v8")
                    }
                } else if (Build.SUPPORTED_32_BIT_ABIS[0]?.equals("armeabi-v7a") == true) {
                    // armv7a (32bit) device
                    Log.d(logTag, "Loading liblobstachatlm_v7a.so")
                    System.loadLibrary("lobstachatlm_v7a")
                } else {
                    Log.d(logTag, "Loading default liblobstachatlm.so")
                    System.loadLibrary("lobstachatlm")
                }
            } else {
    ...
    object DefaultInferenceParams {
        ...
        val chatTemplate: String =
            "{% for message in messages %}{% if loop.first and messages[0]['role'] != 'system' %}{{ '<|im_start|>system You are a helpful AI assistant named LobstaChat, trained by Rob Lobsta<|im_end|> ' }}{% endif %}{{'<|im_start|>' + message['role'] + ' ' + message['content'] + '<|im_end|>' + ' '}}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant ' }}{% endif %}"
    }
    ...
    private fun verifyHandle() {
        check(nativePtr != 0L) { "Model is not loaded. Use LobstaChatLM.load to load the model" }
    }
